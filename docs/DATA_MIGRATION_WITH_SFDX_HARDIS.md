...
- Authenticated target org (e.g., `ENOS-Dev`)
...
1. **Category\_\_c** - Product categories with descriptions and sort order
...
```bash
sf hardis:org:configure:data -o ENOS-Dev
```
...
```bash
sf hardis:org:data:export -o ENOS-Dev -p data/sfdmu
```
