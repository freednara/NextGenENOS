...
# Script to update all LWC meta.xml files with correct object specifications
# Salesforce doesn't allow '*' for object targeting - must specify individual objects
...
echo -e "${BLUE}Updating LWC meta.xml files with correct object specifications...${NC}"
...
update_component_meta() {
    ...
    if [ -f "$meta_file" ]; then
        ...
        # Create the updated meta.xml content with specific objects
        cat > "$meta_file" << EOF
...
<LightningComponentBundle xmlns="http://soap.sforce.com/2006/04/metadata">
    ...
    <targetConfigs>
        <!-- Allow use on relevant record types -->
...
echo -e "${GREEN}🎉 All LWC meta.xml files updated with correct object specifications!${NC}"
echo -e "${BLUE}Components can now be used on:${NC}"
echo -e "  • Lightning App Builder pages"
echo -e "  • Record pages (Product2, Account, Contact, Order, Cart__c, Cart_Item__c)"
echo -e "  • Home pages"
