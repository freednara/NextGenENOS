...
import getAllProducts from "@salesforce/apex/ENOSController.getAllProducts";
...
export default class ProductCatalog extends NavigationMixin(LightningElement) {
  ...
  get categoryOptions() {
    ...
    // Extract unique categories from products (use Family field as fallback)
    const categories = new Set();
    this.allProducts.forEach((product) => {
      // Try Category__c first, then fall back to Family
      const category = product.Category__c || product.Family;
      if (category) {
  ...
  async loadProducts() {
    this.loading = true;
    try {
      // Simplified call to getAllProducts without complex parameters
  ...
  applyFilters(products) {
    ...
    // Category filter using Category__c or Family field
    if (this.selectedCategory) {
      filtered = filtered.filter((p) => {
        const category = p.Category__c || p.Family;
        return (
