...
export default class MyQuotes extends LightningElement {
  ...
  get totalValue() {
    return this.quotes.reduce(
      (sum, q) => sum + (q.GrandTotal || q.TotalPrice || 0),
      0
    );
  }
  ...
  async loadQuotes() {
    ...
    try {
      ...
      this.hasError = true;
      this.errorMessage =
        error?.body?.message || error.message || "Unable to load quotes.";
      this.quotes = [];
  ...
  async handleCreateNewQuote() {
    ...
    try {
      if (this.isAccountMode) {
        ...
        this.showToast(
          "Quote Created",
          "Your cart has been converted to a quote.",
          "success"
        );
      }
      ...
      const message =
        error?.body?.message || error.message || "Unable to create quote.";
